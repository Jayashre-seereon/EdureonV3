    ...
    <PageContainer>
      ...
      {addRoomFor && (
        <CrudDialog open onOpenChange={(v) => !v && setAddRoomFor(null)} title={`Add Room — ${addRoomFor}`}
          fields={[
            ...
            { name: "type", label: "Type", type: "select", options: ["Single", "Twin Sharing", "Triple Sharing", "Quad Sharing", "Dormitory"] },
            { name: "capacity", label: "Capacity (beds)", type: "number" },
          ]}
          submitLabel="Add Room"
          onSubmit={(d) => {
            const ref = addRoomFor;
            setBlocks((p) => p.map((b) => b.name !== ref ? b : { ...b, rooms: [...b.rooms, { no: String(d.no), type: String(d.type), capacity: Number(d.capacity) || 2, allocations: [] }] }));
            setAddRoomFor(null);
          }}
        />
      )}
      ...
      {allocFor && (
        <AllocateDialog
          ...
          onAssign={(alloc) => {
            const ref = allocFor;
            setBlocks((p) => p.map((b) => b.name !== ref.block ? b : { ...b, rooms: b.rooms.map((r) => r.no !== ref.room.no ? r : { ...r, allocations: [...r.allocations, alloc] }) }));
...
function AllocateDialog({ block, room, students, onClose, onAssign }: {
  ...
  students: Array<{ id: string; name: string; phone?: string; parent?: string }>;
  onClose: () => void; onAssign: (a: Allocation) => void;
}) {
  const usedBeds = new Set(room.allocations.map((a) => a.bed));
  const beds = ["A", "B", "C", "D", "E"].slice(0, room.capacity).filter((b) => !usedBeds.has(b));
  const [studentId, setStudentId] = useState(students[0]?.id ?? "");
  const [bed, setBed] = useState(beds[0] ?? "A");
  const [checkin, setCheckin] = useState(new Date().toISOString().slice(0, 10));
  const [query, setQuery] = useState("");
  const filtered = students.filter((s) => !query || (s.name + s.id).toLowerCase().includes(query.toLowerCase())).slice(0, 50);
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-display">Allocate Student — {block} / {room.no}</DialogTitle>
          <DialogDescription>{room.type} · {beds.length} free bed{beds.length !== 1 ? "s" : ""} available.</DialogDescription>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div>
            <Label className="text-xs">Search Student</Label>
            <Input placeholder="Name or ID…" value={query} onChange={(e) => setQuery(e.target.value)} />
          </div>
          <div>
            <Label className="text-xs">Student</Label>
            <Select value={studentId} onValueChange={setStudentId}>
              <SelectTrigger><SelectValue placeholder="Pick student" /></SelectTrigger>
              <SelectContent className="max-h-72">{filtered.map((s) => <SelectItem key={s.id} value={s.id}>{s.name} · {s.id}</SelectItem>)}</SelectContent>
            </Select>
            <div className="text-[10px] text-muted-foreground mt-1">Only students not already allocated are listed.</div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-xs">Bed</Label>
              <Select value={bed} onValueChange={setBed}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{beds.map((b) => <SelectItem key={b} value={b}>Bed {b}</SelectItem>)}</SelectContent></Select>
            </div>
        ...
        <DialogFooter>
          ...
          <Button className="gradient-primary border-0" disabled={!sel || beds.length === 0} onClick={() => sel && onAssign({ studentId: sel.id, studentName: sel.name, bed, checkin, guardian: sel.phone ?? "" })}>Allocate</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
