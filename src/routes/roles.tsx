...
import { Shield, Plus, Users, Key, History, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
...
function RolesPage() {
  ...
  const submitRole = (d: CrudRecord) => {
    const payload = {
      name: String(d.name),
      level: (d.level as CustomRole["level"]) || "Read only",
      scope: (d.scope as CustomRole["scope"]) || "Institute",
      desc: String(d.desc || ""),
    };
  ...
  return (
    <PageContainer>
      <PageHeader eyebrow="System" title="Roles & Permissions"
        ...
        actions={<Button size="sm" className="gradient-primary border-0" onClick={() => { setRoleEdit(null); setRoleOpen(true); }}><Plus className="h-4 w-4" />New Role</Button>}
      />

      <CrudDialog open={roleOpen} onOpenChange={setRoleOpen}
        title={roleEdit ? "Edit Role" : "Create Custom Role"}
        initial={roleEdit ? { name: roleEdit.name, level: roleEdit.level, scope: roleEdit.scope, desc: roleEdit.desc } : undefined}
        fields={[
          { name: "name", label: "Role Name" },
          { name: "level", label: "Access Level", type: "select", options: ["Read only", "Read/Write", "Full Admin"] },
          { name: "scope", label: "Scope", type: "select", options: ["Institute", "Department", "Class", "Self"] },
        ...
        <TabsContent value="custom" className="mt-4">
          <Card className="border-border/60">
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Level</TableHead><TableHead>Scope</TableHead><TableHead>Description</TableHead><TableHead className="w-10"></TableHead></TableRow></TableHeader>
                <TableBody>
                  {customRoles.length === 0 && <TableRow><TableCell colSpan={5} className="text-center text-sm text-muted-foreground py-10">No custom roles yet. Click "New Role" to create one.</TableCell></TableRow>}
                  {customRoles.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="font-medium">{r.name}</TableCell>
                      <TableCell><Badge variant="secondary">{r.level}</Badge></TableCell>
                      <TableCell>{r.scope}</TableCell>
                      <TableCell className="text-xs text-muted-foreground max-w-xs truncate">{r.desc}</TableCell>
                      <TableCell>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => { setRoleEdit(r); setRoleOpen(true); }}><Pencil className="h-4 w-4" />Edit</DropdownMenuItem>
